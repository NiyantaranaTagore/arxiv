...
This project is a Python-based pipeline designed to analyze a given academic paper and identify semantic similarities within a corpus of other documents. It can dynamically fetch relevant papers from arXiv or use a local directory of PDFs, compare them sentence-by-sentence, and generate a clear, color-coded PDF report highlighting the sections with significant semantic overlap.
...
The pipeline follows a high-level, five-step process orchestrated by `main.py`:

1.  **Configuration**: Loads all parameters from `configs/config.yaml`.
2.  **Load & Parse Source**: The source document (`input_doc_path`) is loaded, and its title and abstract are extracted and split into sentences using utilities from `utils/text_utils.py`.
3.  **Acquire Corpus**: Based on the `use_arxiv_corpus` flag, the pipeline either:
    -   Queries the arXiv API for relevant papers using `pipeline/arxiv_fetcher.py`.
    -   Scans the local `corpus_dir` for PDF documents.
4.  **Analyze & Compare**: The `SimilarityAnalyzer` class in `pipeline/similarity_analyzer.py` loads a sentence-transformer model, encodes all sentences into vector embeddings, and calculates the cosine similarity between the source and corpus sentences.
5.  **Generate Report**: If any sentence pairs exceed the `similarity_threshold`, the `generate_report` function in `pipeline/reporting.py` creates a PDF report in the `output_dir`, visually highlighting the similar sentences and listing the corresponding sources.

## Project Structure

```
semantic-similarity-pipeline_v2/
├── assets/
...
├── pipeline/
│   ├── arxiv_fetcher.py    # Handles searching and fetching papers from arXiv.
│   ├── data_loader.py      # Loads the source document and configuration.
│   ├── reporting.py        # Generates the final PDF report.
│   └── similarity_analyzer.py # Core logic for model loading, embedding, and similarity calculation.
├── utils/
